...
public class query_Update_DOT_THUCHIEN_TANG_TAISAN {
	...
	public String getString_Capnhat_DotTangTaisan(DOT_THUCHIEN_TANG_TAISAN dtt) {
		try {
			String nguontang = dtt.getMA_NGUONTANG() < 0 ? ", MA_NGUONTANG = null"
					: (dtt.getMA_NGUONTANG() == 0 ? " " : ", MA_NGUONTANG = '" + dtt.getMA_NGUONTANG() + "'");
			String result = "UPDATE DOT_THUCHIEN_TANG_TAISAN  SET TEN_DOT_TANG='" + dtt.getTEN_DOT_TANG()
					+ "', LY_DO_TANG='" + dtt.getLY_DO_TANG() + "', MO_TA = '" + dtt.getMO_TA() + "' " + nguontang
					+ "  WHERE MA_DOT_TANG= '" + dtt.getMA_DOT_TANG() + "'";
			return result;
		} catch (Exception e) {
			return null;
		}
